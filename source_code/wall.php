...
function sendstatus() {
  ...
  if(xmlhttp) {
    ...
    xmlhttp.send("status="+status.value);
  }
}
...
<?php
$sql="SELECT * FROM wall_data WHERE email = '".$_SESSION['email']."'";
...
while($row = mysql_fetch_array($result))
  {
  echo "<tr>";
  echo "<td ><a style='color:#9999FF;' href='user.php?email=" . $row['email'] ."' style='color:black;'>" . $row['email'] . "</a></td>";
...
<p> <span id="txtHint"></span></p> 
 ...
 <a href="profile.php" ><button type="button" class="wall" ><b>profile</b></button></a>
